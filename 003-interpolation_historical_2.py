...
def only (casename, dirs, int_year, end_year, int_mon, end_mon, regrid_hor, regrid_ver, vars_plev_in, vars_alev_in):
    ...
    ### Part 1: regrid from SE to FV grid for all monthly dataset
			...
			if (regrid_ver == "T"):
				...
				outdir="/GPFS/cess9/qinyi/CMOR3/mid-data/"
				print("outdir="+outdir)
				cmd = 'sh ./cdo-ml2pl-params.sh %s %s %s %i %i %i %i %s %s' %(datadir, casename[icase], outdir, int_year[icase], end_year[icase], int_mon[icase], end_mon[icase], vars_plev_in[icase], vars_alev_in[icase])
				os.system(cmd)
