...
ncfile='/GPFS/cess9/qinyi/CMOR3/mid-data/'+casename[0]+'.cam.h0.'+str(year[0])+'-'+str("%02d" % mon[0])+'_plev19.nc'
print(ncfile)
...
for ivar in range(len(varin4d_alev)):
	for icase in range(len(casename)):
		...
		for iyear in year:
			...
			for imon in mon:
				print("icase=, iyear=, imon=",str(icase), str(iyear),str(imon))
				ncfile='/GPFS/cess9/qinyi/CMOR3/mid-data/'+casename[icase]+'.cam.h0.'+str(iyear)+'-'+str("%02d" % imon)+'_plev19.nc'
				print(ncfile)
				fnc=Dataset(ncfile,'r')
				vars=fnc.variables.keys()
